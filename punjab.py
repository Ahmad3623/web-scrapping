...
for i in all_rows:
    if(len(i)==13):
        turnout.append(str(i[1]))
        candidate.append(str(i[2]))
        name.append(str(i[0]))
        party.append(str(i[4]))
        votes.append(str(i[5]))
        Per.append(str(i[6]))
        candidateru.append(str(i[7]))
        party_ru.append(str(i[9]))
...
to_csv=df.to_csv('C:/Users/ahmad/Desktop/runnerupp.csv')
...
df=pd.DataFrame(dictionary)
to_csv=df.to_csv('C:/Users/ahmad/Desktop/winnerp.csv')
...
#Election Prediction
dictionary={'turnout':turnout, 'DISTRICT': district, 'NAME':name, 'WINNERCANDIDATE':candidate, 'WINNERPARTY':party, 'WINNERVOTES':votes, 'WINNERPERCENTAGE':Per, 'RUNNERUPVOTES': votes_ru,'RUNNERUPCANDIDATE': candidateru,'RUNNERUPPARTY': party_ru,'RUNNERUPPERCENTAGE': per_ru, 'MARGIN': margin,'percentage_margin': percentage_margin}
df=pd.DataFrame(dictionary)
to_csv=df.to_csv('C:/Users/ahmad/Desktop/db_electionp.csv')